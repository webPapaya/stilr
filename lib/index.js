...
const renderMediaQueryPretty = (className, mediaQueryCSS) =>
  `${ className } {\n${ mediaQueryCSS }}\n`;

const renderMediaQueryMinified = (className, mediaQueryCSS) =>
  `${ className }{${ mediaQueryCSS }}`;

const renderCSSDeclarationPretty = (className, markup)  =>
  `.${ className } {\n${ markup.split(';').join(';\n')}}\n`;

const renderCSSDeclarationMinified = (className, markup) =>
  `.${ className }{${ markup }}`;


const renderers = {
  default: {
    renderCSSDeclaration: renderCSSDeclarationMinified,
    renderMediaQuery: renderMediaQueryMinified
  },
  pretty: {
    renderCSSDeclaration: renderCSSDeclarationPretty,
    renderMediaQuery: renderMediaQueryPretty
  }
};
    ...
    for ( const entry of stylesheetEntries ) {
      ...
      if (isMap) {
        ...
        mediaQueries += options.pretty
          ? renderers.pretty.renderMediaQuery(className, mediaQueryCSS)
          : renderers.default.renderMediaQuery(className, mediaQueryCSS);
        continue;
      }

      const markup = createMarkup( styles );
      css += options.pretty
        ? renderers.pretty.renderCSSDeclaration(className, markup)
        : renderers.default.renderCSSDeclaration(className, markup);
    }
