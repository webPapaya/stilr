...
import { createCssRules, renderers, stringifyFnForSelector } from '../renderer';
...
describe('renderer', () => {
  ...
  describe('#stringifyFnForSelector', () => {
    describe('when options pretty', () => {
      it('returns correct fn for Media Query', () => {
        const selector = '@media screen';
        const stringifyFn = stringifyFnForSelector(selector);
        assert.equal(stringifyFn, renderers.pretty.stringifyMediaQuery);
      });

      it('returns correct fn for Keyframe Animation', () => {
      ...
      it('returns correct fn for css class', () => {
