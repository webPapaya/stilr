...
export default {
  ...
  render(options = { pretty: false }, stylesheet = globalStylesheet ) {
    ...
    for ( const entry of stylesheetEntries ) {
      ...
      const pretty = renderers.pretty;
      const minified = renderers.default;

      const renderer = options.pretty
        ? pretty
        : minified;

      if (isMap) {
        const mediaQueryCSS = this.render(options, stylesheet.get( className ) );
        mediaQueries += renderer.stringifyMediaQuery(className, mediaQueryCSS);
        continue;
      }

      const markup = createMarkup( styles );
      css += renderer.stringifyCSSDeclaration(className, markup);
    }
