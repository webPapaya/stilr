...
function renderMediaQueryPretty(className, mediaQueryCSS) {
  return `${ className } {\n${ mediaQueryCSS }}\n`;
}
function renderMediaQueryMinified(className, mediaQueryCSS) {
  return `${ className }{${ mediaQueryCSS }}`;
}
function renderCSSDeclarationPretty(className, markup) {
  return `.${ className } {\n${ markup.split(';').join(';\n')}}\n`;
}
function renderCSSDeclarationMinified(className, markup) {
    ...
    for ( const entry of stylesheetEntries ) {
      ...
      if (isMap) {
        ...
        mediaQueries += options.pretty
          ? renderMediaQueryPretty(className, mediaQueryCSS)
      ...
      css += options.pretty
        ? renderCSSDeclarationPretty(className, markup)
